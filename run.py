...
def uaku():
	try:
		...
		a=requests.get('https://github.com/shinZy378/Xyzv2/blob/main/UA.txt').text
		ua=open('.UA.txt','w')
...
PROXY=AKTIF'''
	oi = nel(tekz(ban,justify='center',style='green'), style='green')
...
def login_lagi334():
	banner()
	sky = '[bold green][01] LOGIN COOKIE V1\n[02] LOGIN COOKIE V2[/bold green]'
	sky2 = nel(sky, style='red')
#	cetak(nel(sky2,title='[bold green] • LOGIN MENU • [/bold green]'))
#	pil=input('[•] Choose : ')
	pil='1'
	if pil in ['1','01']:
		try:
...
# DAFTAR KATA
def passwrd():
	ler = '# PROSES CRACK DI MULAI, TEKAN CTRL+Z UNTUK BERHENTI'
	sol().print(mark(ler, style='green'))
	krek = '[•] OK HASIL DISIMPAN DI : INTERNAL MEMORY/4MBF-DATA/OK/%s\n[•] CP HASIL DISIMPAN DI IN : INTERNAL MEMORY/4MBF-DATA/CP/%s\nAKTIFKAN MODE PESAWAT SETIAP 500-1000 ID, JIKA MASIH TERKENA SPAM IP COB AKTIFKAN VPN SELAMA 1 MENIT, ATAU BISA JUGA GANTI APN KALIAN'%(okc,cpc)
